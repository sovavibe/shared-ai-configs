...
```bash
bd create --title="[{PREFIX}-XXX] Define types for Feature" --type=task --priority=2
bd create --title="[{PREFIX}-XXX] Create useFeature hook" --type=task --priority=2
bd create --title="[{PREFIX}-XXX] Build Feature UI component" --type=task --priority=2
bd create --title="[{PREFIX}-XXX] Add tests for Feature" --type=task --priority=2
```
