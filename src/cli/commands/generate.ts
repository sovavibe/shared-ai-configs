...
/**
 * Clean all generated directories and files before regenerating.
 * Only removes files that are typically in .gitignore pattern (generated by this tool).
 *
 ...
 */
function cleanGeneratedFiles(outputDir: string, dryRun?: boolean): void {
  logger.header('Cleaning generated files');

  // Directories to remove
  const dirsToClean = ['.claude', '.cursor', '.perles'];
...
export async function generateCommand(options: GenerateOptions): Promise<void> {
  // Clean generated files first if --clean flag is set
  if (options.clean) {
    cleanGeneratedFiles(options.output, options.dryRun);
  }
...
async function updateGitignore(
  ...
  const entries: string[] = [
    ...
    '',
    '# AI Project Config (version-controlled)',
    '# .ai-project.yaml = Team configuration, tracked in git',
    '',
    '# AI Environment (secrets/API keys - NOT version-controlled)',
    '.env.aiproject',
    '',
